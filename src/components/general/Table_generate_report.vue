<template>
	<div>
		...
		<el-card style="margin-top:19px;">历史文档</el-card>
		<el-table
			:data="tableData"
			style="width: 100%"
...
<script>
	...
	mounted(){
		this.$axios.get('/api/v1/get_report')
		.then(response => {
	...
	methods: {
		...
		down(date, project, branch){
			...
			this.$axios({
				method : 'post',
				url : '/api/v1/transmission_report',
				data:{
					date : date,
					project : project,
					branch : branch
				...
				// 注意页面缓存
				if (res.data.size < 19) {
					this.$alert('当日未生成.xls文件', 'Warning', {
					...
					var filename = date + ' ' + project  + ' Perfdog 数据对比.xls'
